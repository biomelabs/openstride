...
/* Timer period in µs: 1 000 000 / 416 = 2403 µs */
#define IMU_PERIOD_US (1000000 / IMU_TARGET_ODR_HZ)
...
/*
 * Timer fires at the target ODR; the poll thread wakes on each tick.
 * Using a fixed-period timer rather than k_sleep(period) means fetch
 * time is not added to the sleep, so we actually hit 416 Hz instead
 * of drifting to ~200 Hz.
 *
 * The LSM6DS3TR-C INT1 pin is defined in the overlay but its physical
 * connection is unverified, so we do not use SENSOR_TRIG_DATA_READY
 * (same situation as the ADXL375 whose DRDY line never asserted).
 */
static K_SEM_DEFINE(imu_tick_sem, 0, 1);
static K_TIMER_DEFINE(imu_tick_timer, NULL, NULL);

static void imu_timer_fn(struct k_timer *timer)
{
	ARG_UNUSED(timer);
...
static void imu_poll_fn(void *p1, void *p2, void *p3)
{
	...
	LOG_INF("Poll thread started at %u Hz (period %u µs)",
		IMU_TARGET_ODR_HZ, IMU_PERIOD_US);

	k_timer_init(&imu_tick_timer, imu_timer_fn, NULL);
	k_timer_start(&imu_tick_timer, K_NO_WAIT, K_USEC(IMU_PERIOD_US));
...
#if DT_NODE_EXISTS(DT_ALIAS(accel0))
		/*
		 * ADXL375 is polled here — interrupt-driven mode was attempted
		 * but the DRDY line never asserted (likely not wired on this board).
		 * At 400 Hz ODR the adxl345 driver's busy-poll collapses to one
		 * iteration instead of ~30.
		 */
		if (ext_accel_ready) {
			int ext_err = sensor_sample_fetch(ext_accel_dev);

			if (ext_err == 0) {
				ext_err |= sensor_channel_get(ext_accel_dev,
							      SENSOR_CHAN_ACCEL_X, &ax);
				ext_err |= sensor_channel_get(ext_accel_dev,
							      SENSOR_CHAN_ACCEL_Y, &ay);
				ext_err |= sensor_channel_get(ext_accel_dev,
							      SENSOR_CHAN_ACCEL_Z, &az);
			}
			...
			if (got_ext_accel && !ext_accel_fetch_ok) {
				...
				LOG_WRN("External accel fetch failed (%d), falling back to internal",
					ext_err);
...
void imu_stop(void)
{
	...
	k_sem_give(&imu_tick_sem); /* unblock the thread if it's waiting */
